...
test('podpisy: BankID přes Ready connection bez ostrého adapteru vrací poctivou hlášku (422) a stav zůstává', async ({
  page,
}) => {
  ...
  await page.route(API, async (route: Route) => {
    ...
    // Backend s BankID connection bez ostrého adapteru vrací 422 (ProblemDetails.detail nese „adapter").
    if (method === 'POST' && path === '/verified-signing/envelopes/env-1/send') {
      return route.fulfill({
        status: 422,
        json: { title: 'Runtime adapter', detail: 'BankID runtime adapter is not enabled yet.' },
      })
    }
    return route.fulfill({ status: 404, json: { title: `Unhandled ${method} ${path}` } })
  })
