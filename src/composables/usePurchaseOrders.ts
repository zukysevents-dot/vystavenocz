...
export interface SupplierInvoice {
  ...
  currency: string
  note: string | null
  confirmedReceiptTotal: number | null
  amountMatchesConfirmedReceipts: boolean | null
  createdAt: string
}
...
export function usePurchaseOrders() {
  ...
  const create = (input: CreatePurchaseOrderInput): Promise<PurchaseOrder> =>
    http.post('/purchase-orders', input)
  const send = (id: string): Promise<PurchaseOrder> => http.post(`/purchase-orders/${id}/send`)
  const sendEmail = (
    id: string,
    input: { to?: string | null; message?: string | null },
  ): Promise<PurchaseOrder> => http.post(`/purchase-orders/${id}/send-email`, input)
  const cancel = (id: string): Promise<PurchaseOrder> => http.post(`/purchase-orders/${id}/cancel`)
  const createReceiptDraft = (id: string, input: ReceiptDraftInput): Promise<StockDocument> =>
    http.post(`/purchase-orders/${id}/receipt-drafts`, input)
  const supplierProducts = (supplierId: string): Promise<SupplierProduct[]> =>
    http.get(`/purchase-orders/suppliers/${supplierId}/products`)
  const upsertSupplierProduct = (
    supplierId: string,
    input: SupplierProductInput,
  ): Promise<SupplierProduct> =>
    http.put(`/purchase-orders/suppliers/${supplierId}/products`, input)
  const removeSupplierProduct = (supplierId: string, productId: string): Promise<void> =>
    http.del(`/purchase-orders/suppliers/${supplierId}/products/${productId}`)
  const suggestions = (
    supplierId: string,
    locationId?: string | null,
  ...
  const createSupplierInvoice = (input: SupplierInvoiceInput): Promise<SupplierInvoice> =>
    http.post('/purchase-orders/supplier-invoices', input)
