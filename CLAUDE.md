...
- **Veřejný web (landing):** komunikuje Vystaveno jako modulární provozní systém (gastro-first), ne jen fakturaci. Aplikace není veřejně spuštěná — z veřejného webu se NEpropaguje login/registrace (routy existují, ale nevedou na ně odkazy); obchodní CTA vedou na e-mail přes `src/lib/landing-cta.ts` (demo, early access, zájem o modul/sestavu). `/akce` = early access nabídka, ceny jsou označené jako orientační do spuštění, nehotové funkce mají label „připravujeme". Obsah: `src/components/landing/`, SEO `src/lib/seo.ts`, FAQ `src/lib/faq-data.ts`. Po veřejném spuštění režim zrušit a CTA vrátit na registraci.
