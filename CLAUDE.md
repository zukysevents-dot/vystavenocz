...
- **Provozní přehled (manažerský dashboard):** `src/pages/ProvozniPrehledPage.vue` (`/app/provozni-prehled`, modul `reporting`, role Owner/Admin/Manager) čte manažerskou analytiku prodejů přes `src/composables/usePosReports.ts` z backendu `GET /api/v1/pos-reports/summary`, `/revenue` a `/costs`. Typy + čistý helper `posReportRange` (rozsahy z lokálního data) jsou v `src/lib/posReports.ts`. Ukazuje KPI tržeb a plateb, graf tržeb po dnech (`BarChart`), nejprodávanější položky a blok `Marže a food cost` s hrubou marží, food cost %, odhadovaným nákladem, tržbami mimo katalog a produkty s nejvyšším food costem; filtr období a provozovny. Jen API režim. Backend (oprávnění `pos.reports`, Owner/Admin/Manager) agreguje jen dokončené prodeje konzistentně s uzávěrkou dne; `/costs` používá aktuální receptury/BOM a nákupní ceny jako MVP odhad nákladu.
