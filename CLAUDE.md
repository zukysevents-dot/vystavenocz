...
- **Gastro receptury/BOM:** editor je v `Sklad / katalog` jako akce u produktu (`ProductRecipeDialog.vue`) a volá `GET/PUT/DELETE /api/v1/products/{productId}/recipe`. Automatické odečítání surovin při prodeji je navazující backend krok ve `vystaveno-api`.
