...
- **Cenové hladiny, akce a věrnost:** backend `vystaveno-api` má persistentní CRUD `GET/POST/PUT/DELETE /api/v1/price-levels`, `GET/POST/PUT/DELETE /api/v1/promotions` a serverový výpočet `POST /api/v1/promotions/calculate` (modul `loyalty`, oprávnění `loyalty.read/manage`). Backend PR #216 napojuje automatická promo pravidla do ostrého checkoutu přes sdílený `SaleService.PersistSaleAsync`; backend PR #217 přidává volitelný `PriceLevelId` pro POS i platbu účtu a ukládá oddělený snapshot dopadu cenové hladiny vs promo slevy. Frontend `src/composables/usePromotions.ts` mapuje backend enumy na `src/lib/promotions.ts` typy a `src/pages/AkceProvozuPage.vue` (`/app/akce-ceny`) spravuje hladiny/pravidla. `PokladnaPage.vue` a `RestauracePage.vue` načítají cenové hladiny, před platbou volají serverový náhled `/promotions/calculate`, v souhrnu ukazují dopad hladiny/akcí a posílají `priceLevelId` přes `useSales.create` nebo `useOrders.pay`; split platba osoby v jednom účtu zatím cenovou hladinu samostatně nevolí. `src/lib/promotions.ts` zůstává mock/fallback a čistý sdílený model. Další krok je věrnostní body earn/redeem a případné VIP propojení na zákaznický účet/QR identitu.
