...
- **Pokladna / EAN sken:** `src/pages/PokladnaPage.vue` má pole `Sken / EAN` pro HW/Bluetooth čtečku v režimu klávesnice i tlačítko `Kamera`, které používá sdílený `CameraScanner.vue` s vlastní POS hláškou. Přidání hledá přes exact `Product.ean` (`findByEan`); neznámý EAN a duplicitní EAN zobrazují toast a nepřidávají položku. E2E `e2e/pokladna-scan.spec.ts` hlídá přidání položky a navýšení množství opakovaným skenem.
