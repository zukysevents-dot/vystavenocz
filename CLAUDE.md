...
- **Stav integrací:** `src/lib/integration-readiness.ts` je frontendový zdroj pravdy pro sekci `Integrace a exporty` v `Nastavení firmy`. Backend #219 přidal foundation endpointy `GET /api/v1/integrations/exports`, `GET /api/v1/integrations/terminal-payments` a `GET /api/v1/integrations/print-jobs` za modulem `integrations`; backend #220 přidal `GET /api/v1/integrations/exports/download` a první reálný účetní adaptér `target=Pohoda&format=Xml`. Frontend `src/composables/useIntegrations.ts` je mapuje do panelu v nastavení: UI v API režimu se zapnutým modulem ukazuje Generic CSV/Pohoda XML účetní export, poslední/pending terminálové platby a tiskovou frontu; mimo API režim zůstává readiness přehled. Pohoda je souborový XML export pro ruční import, ne živá synchronizace. Nepiš do UI ani dokumentace, že existuje reálná terminálová brána, lokální tiskový agent, Money/SuperFaktura renderer nebo partnerské API, dokud nevznikne skutečný adaptér.
