...
- **Cenové hladiny, akce a věrnost:** backend `vystaveno-api` má persistentní CRUD `GET/POST/PUT/DELETE /api/v1/price-levels`, `GET/POST/PUT/DELETE /api/v1/promotions` a serverový výpočet `POST /api/v1/promotions/calculate` (modul `loyalty`, oprávnění `loyalty.read/manage`). Frontend `src/composables/usePromotions.ts` mapuje backend enumy na `src/lib/promotions.ts` typy a `src/pages/AkceProvozuPage.vue` (`/app/akce-ceny`) už umí spravovat hladiny/pravidla a náhled počítá serverem v API režimu. `src/lib/promotions.ts` zůstává mock/fallback a čistý sdílený model. Další krok je napojit pricing service do ostrého `Pokladna`, `Restaurace` a veřejných/QR objednávek tak, aby checkout cenu vždy potvrzoval backend.
