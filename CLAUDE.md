...
- **Stav integrací:** `src/lib/integration-readiness.ts` je frontendový zdroj pravdy pro sekci `Integrace a exporty` v `Nastavení firmy`. Backend #219 přidal foundation endpointy `GET /api/v1/integrations/exports`, `GET /api/v1/integrations/terminal-payments` a `GET /api/v1/integrations/print-jobs` za modulem `integrations`; frontend `src/composables/useIntegrations.ts` je mapuje do panelu v nastavení. UI v API režimu se zapnutým modulem ukazuje Generic CSV účetní export, poslední/pending terminálové platby a tiskovou frontu; mimo API režim zůstává readiness přehled. Nepiš do UI ani dokumentace, že existuje reálná terminálová brána, lokální tiskový agent, POHODA/Money/SuperFaktura renderer nebo partnerské API, dokud nevznikne skutečný adaptér.
