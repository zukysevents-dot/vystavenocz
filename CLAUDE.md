...
- **Cenové hladiny, akce a věrnost:** `src/lib/promotions.ts` je čistý výpočtový model pro cenové hladiny, časové/kategoriové/produktové akce a věrnostní body; `src/pages/AkceProvozuPage.vue` (`/app/akce-ceny`, modul `loyalty`) je frontend preview modelu pro ladění pravidel. Zatím nejde o persistentní backend správu pravidel. Další krok je backend CRUD pravidel + napojení do `Pokladna`, `Restaurace` a veřejných/QR objednávek tak, aby konečnou cenu vždy počítal server.
