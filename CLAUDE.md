...
- **Modulární runtime základ:** `/me` vrací `modules` + `features`; frontend je drží v `src/stores/auth.ts` a app shell filtruje navigaci/routy přes `src/lib/modules.ts`. Defaultně jsou zapnuté všechny moduly.
