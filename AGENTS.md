...
Veřejné objednávky: route `/objednavka/:slug` (`PublicOrderPage.vue`) používá `usePublicOrders` a public endpointy `GET /public/{slug}/menu` + `POST /public/{slug}/orders` bez JWT. Bez query `table` zákazník vybírá menu do košíku, zvolí `pickup`/`delivery`, vyplní kontakt a objednávka jde rovnou do KDS; ceny vždy počítá backend z katalogu. QR objednávka ke stolu používá `/objednavka/:slug?table=<tableId>&name=<tableName>`, schová volbu výdej/rozvoz, odešle `tableId` a backend PR #182 objednávku uloží do dine-in účtu stolu; pokud už účet existuje, přidá do něj nové položky. `Mapa stolů` generuje pro vybraný stůl QR kód a kopírovatelný odkaz z `Company.publicSlug` vraceného `/company`.
...
## Veřejný web (landing) — režim před spuštěním

Veřejné stránky (`/`, `/funkce`, `/cenik`, `/srovnani`, `/faq`, `/akce`, `/nase-sliby`, `/clanky`) komunikují Vystaveno jako **modulární provozní systém** (gastro-first: stoly, kuchyně, receptury, food cost, zrcadlo, inventury, uzávěrky; plus služby, řemeslo, obchod), ne jen fakturaci. Obsah žije v `src/components/landing/`, SEO v `src/lib/seo.ts`, FAQ v `src/lib/faq-data.ts`, ceníková data v `src/lib/pricing.ts`.

Aplikace zatím **není veřejně spuštěná** — z veřejného webu se NEpropaguje login ani registrace (routy `/prihlaseni` a `/registrace` existují dál, jen na ně nevedou odkazy). Všechna obchodní CTA vedou na e-mail přes `src/lib/landing-cta.ts` (demo, early access, zájem o modul/sestavu) — nové CTA přidávej přes tento helper. Stránka `/akce` = early access nabídka. Ceny na webu jsou označené jako orientační do spuštění; co není hotové, má na webu label „připravujeme" (žádné falešné sliby). Po veřejném spuštění tento režim zrušit a CTA vrátit na registraci.
