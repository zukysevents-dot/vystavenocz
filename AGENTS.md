...
Cenové hladiny, akce a věrnost: `src/lib/promotions.ts` je čistý výpočtový model pro cenovou hladinu (`PriceLevel`), časová/pravidlová promo pravidla (`PromotionRule`) a věrnostní body/redeem. `src/pages/AkceProvozuPage.vue` (`/app/akce-ceny`, modul `loyalty`) je frontend preview modelu nad ukázkovým účtem; zatím nejde o persistentní backend správu pravidel. Další krok je backend CRUD pravidel + napojení do `Pokladna`, `Restaurace` a veřejných/QR objednávek tak, aby konečnou cenu vždy počítal server.
