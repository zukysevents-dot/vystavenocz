...
Stav integrací: `src/lib/integration-readiness.ts` je frontendový zdroj pravdy pro sekci `Integrace a exporty` v `Nastavení firmy`. Rozlišuje `ready` exporty (ISDOC/CSV faktury, Z-reporty), `manual` provozní kroky (platební terminál, tisk přes prohlížeč), `export` režim (POHODA/Flexi přes soubory) a `planned` věci (partnerské API). UI ukazuje souhrn použitelné v provozu / připravené exporty / čeká na konektor. Nepiš do UI ani dokumentace, že terminál, přímý tisk, POHODA, Flexi nebo partnerské API běží přímou synchronizací, dokud nevznikne skutečný konektor.
...
Cenové hladiny, akce a věrnost: backend `vystaveno-api` má persistentní CRUD `GET/POST/PUT/DELETE /api/v1/price-levels`, `GET/POST/PUT/DELETE /api/v1/promotions` a serverový výpočet `POST /api/v1/promotions/calculate` (modul `loyalty`, oprávnění `loyalty.read/manage`). Backend PR #216 už napojuje automatická promo pravidla do ostrého checkoutu přes `SaleService.PersistSaleAsync`, takže `Pokladna`, `Restaurace` i QR/public objednávky při následné platbě používají serverovou finální cenu a snapshot `OriginalUnitPrice` + `PromotionDiscount` pro historii/účtenku/Z-report. Frontend `src/composables/usePromotions.ts` mapuje backend enumy na `src/lib/promotions.ts` typy a `src/pages/AkceProvozuPage.vue` (`/app/akce-ceny`) umí spravovat hladiny/pravidla a náhled počítá serverem v API režimu. `src/lib/promotions.ts` zůstává mock/fallback a čistý sdílený model. Další krok je napojit cenové hladiny/VIP do ostrého checkoutu (výběr hladiny na účtu/prodeji + backend `PriceLevelId` v pricing službě) a potom věrnostní body earn/redeem.

Ověřené podpisy / Bank iD podpis: produktová roadmapa počítá se samostatným placeným modulem pro ověřené podepisování dokumentů napříč Vystavenem (faktury, smlouvy, předávací protokoly, zakázkové protokoly, rezervace, servisní dokumenty a nahrané PDF). Bank iD je první český provider k prověření, ale architektura musí být providerová, aby šel později doplnit Signi/DocuSign/DigiSign nebo interní podpis. Modul patří do integrací + dokumentové vrstvy, ne jen do gastra; musí ukládat stav workflow, hash dokumentu, provider transaction id, povolený identity payload, timestampy, podepsané PDF a auditní stopu. Zatím není implementováno.
