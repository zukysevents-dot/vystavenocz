...
Veřejné objednávky: route `/objednavka/:slug` (`PublicOrderPage.vue`) používá `usePublicOrders` a public endpointy `GET /public/{slug}/menu` + `POST /public/{slug}/orders` bez JWT. Zákazník vybírá menu do košíku, zvolí `pickup`/`delivery`, vyplní kontakt a objednávka jde rovnou do KDS; ceny vždy počítá backend z katalogu.
