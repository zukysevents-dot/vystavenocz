...
7. **Sklad / inventura / zrcadlo** — v `Zásoby`:
   ...
   - jednu kladnou šarži dej do `Karantény` s důvodem. Očekávání: fyzický stav se nezmění, `Blokováno` vzroste a `K dispozici` klesne; šarži nelze vydat, prodat, rezervovat, přesunout ani doplnit příjmem pod stejným číslem. Ověř `Historii`, potom šarži uvolni. `Reklamovaná` je konečná a nejde vrátit,
   ...
   - na mobilním viewportu spusť další inventuru. Očekávání: první kolo je slepé, EAN scan přičítá kusy, zavření a otevření obnoví průběh na stejném zařízení, druhé kolo ukáže pouze rozdíly a po úspěchu se otevře protokol s CSV a tiskem/PDF. Síťový retry se stejným průběhem nesmí vytvořit druhý doklad ani korekční pohyb.
...
- **Ostré / hotové na VPS:** login a moduly, gastro POS prodej, restaurace/stoly/kuchyně, uzávěrka + Z-report, sklad/inventura/zrcadlo/naskladnění včetně dodavatelů, nákupních objednávek, dílčích příjmů, šarží, expirací a FEFO, akce a ceny, věrnost, audit, veřejné/QR objednávky, dokumenty a fotografie u zakázek v persistentním `api_files`, účetní export Generic CSV a Pohoda XML (soubor pro ruční import, ne živá synchronizace), tiskoví agenti (registrace/token/revoke), credential trezor plateb i podpisů (uložit/rotovat/smazat/revokovat klíče).
