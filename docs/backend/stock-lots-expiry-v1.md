...
- Šarže má stav `Active`, `Quarantined`, `Blocked` nebo `Recalled`. Jen `Active` vstupuje do FEFO a disponibilní zásoby; `Recalled` je konečný stav.
- Každá změna stavu vyžaduje důvod a ukládá se do neměnné historie s časem a autorem. Stav platí celofiremně i tehdy, když šarže leží ve více pobočkách.
...
- `ZasobyPage.vue` + `StockLotsPanel.vue`: mobilní karty zůstatků, hledání, produkt, pobočka, datum expirace a stav, přesný odpis, karanténa/blokace/reklamace s historií, ruční příjem a výběr aktivní šarže pro výdej/přesun. Přesný výběr stránkuje i přes více než 100 aktivních šarží.
...
| `GET`  | `/api/v1/inventory/stock-lots?productId=&locationId=&expiresTo=&positiveOnly=&search=&status=&page=&pageSize=` | zůstatky a stavy    |
| `POST` | `/api/v1/inventory/stock-lots/{stockLotId}/status`                                                             | změna stavu a důvod |
| `GET`  | `/api/v1/inventory/stock-lots/{stockLotId}/status-history`                                                     | neměnná historie    |
...
- Karty i centrální sklad ukazují blokované množství odděleně; dostupnost je fyzický stav minus rezervace minus omezené šarže.
