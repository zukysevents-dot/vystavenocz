...
- mobile-first stocktake workflow with full, partial, cycle, and spot scopes, blind first count, second recount only for variances, EAN/camera entry, CSV/PDF protocol, and safe retry via idempotency; the in-progress concept is backed up locally and, in API mode, synchronized through the tenant-scoped `inventory/stocktake-drafts` API so it can be continued on another device. Revision conflict blocks finalization until the newer server state is explicitly loaded; creating, updating, or deleting the concept never writes the stock ledger, which is changed only by the existing atomic final stocktake endpoint.
