...
- multi-location central stock, shared price lists, and branch comparisons (foundation: locations exist, POS/manager reports accept `locationId`, `/app/konsolidace` compares branch revenue, stock mirror filters by location, backend location-stock balances add location-specific stock balances, and stock levels can be filtered by branch in `Zásoby`. Remaining work: first-class central warehouse UX, shared price lists, and richer branch comparisons for margin/stock/losses.)
