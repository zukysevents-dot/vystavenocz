...
- mobile scanner stocktake (INV-16 V1: blind first pass without prefilled quantities, HW EAN and camera scan, same-device draft per company/user/location, second independent recount only for differences, UUID-idempotent finalization, authoritative protocol CSV and print/PDF; cross-device/server draft synchronization remains a later slice)
