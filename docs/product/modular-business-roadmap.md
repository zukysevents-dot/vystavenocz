...
- suppliers, purchase receipts, purchase prices, average cost, and price history (foundation: backend PR #156 and `Naskladnění` UI create auditable purchase receipt documents; INV-16 adds suppliers, purchase orders with partial/idempotent receipts, and lot/expiry snapshots on receipt lines)
...
- lots/batches, expiry and FEFO (INV-16: irreversible per-product tracking over the existing append-only ledger; manual, purchase-order and production receipts capture lot/expiry, outbound sale/job/manual flows allocate earliest active expiry first, transfer and exact reversals preserve allocations, `Zásoby → Šarže` provides mobile balances, exact expiry write-off and `Active / Quarantined / Blocked / Recalled` safety states with mandatory append-only reasons; restricted balances are excluded from sale, issue, reservation, transfer, jobs, production and availability, while the movement card and CSV expose the allocation audit trail)
...
- mobile scanner stocktake (INV-16 V1: before counting, staff chooses full, partial, cyclic or spot range; partial/cyclic ranges can combine categories and individual products, while spot range includes only explicitly chosen products. The first pass remains blind without prefilled quantities, supports HW EAN and camera scan, persists a same-device draft per company/user/location, requires an independent recount only for differences, uses UUID-idempotent finalization, and provides authoritative protocol CSV and print/PDF. Cross-device/server draft synchronization remains a later slice.)
