...
8. Spusťte `Inventuru` na konkrétní pobočku. Před počítáním vyberte `Úplná`, `Částečná`, `Cyklická` nebo `Namátková kontrola`; u částečné/cyklické kombinujte kategorie a jednotlivé produkty, u namátkové jen konkrétní produkty. Zadejte fyzicky napočítané množství a uložte. `Rozdíl` = realita minus systém, ukazuje se v kusech i v Kč.
...
11. Na mobilním viewportu 390 × 844 spusťte novou inventuru. Ověřte výběr rozsahu: úplná nabízí celý katalog, částečná/cyklická přidá všechny produkty z označené kategorie a namátková nepustí nic mimo ručně označené položky. První kolo je slepé a prázdné, HW EAN/ruční scan přičte jeden kus a kamera je dostupná. Vyplňte alespoň dvě položky, dialog zavřete a znovu otevřete — počty i stabilní průběh musí zůstat. Dokončete první kolo, ve druhém musí být jen rozdílné položky. Po uložení stáhněte protokol CSV a otevřete tisk/PDF. Při simulované chybě POST musí průběh zůstat a opakování nesmí vytvořit druhý doklad/pohyb.
